...
Since CV values are influenced both by sampling effort and the magnitude of the estimated intensity means other methods we also map the lower and upper quantiles corresponding to a 95\% credible interval for each prediction cell (\autoref{fig:intensity-quantiles}).
\begin{figure}[h]
...
\begin{figure}[h]
	\includegraphics[scale=0.6]{figures/matern_posterior.png}
	\caption{Posterior Mat\'ern correlation function}
	\label{fig:matern-posterior}
\end{figure}
This shows some correlation even at distances of 10,000 metres.  However, the northern-most and souther-most boundaries of the study region are approximately 8,000 metres apart.  The model is estimating positive correlation at distances further apart than were observed.  [[I'm not sure if this a problem??]]
...
Finally, we note that the one-stage approach is a useful approach when it comes to representing uncertainty.  All plots here are based on sampling from the joint posterior of all model parameters.  As such they naturally average over the uncertainty in the detection model.  

\clearpage %figure out figure placements later
