...
\begin{figure}[!htb]
	\begin{center}
		...
		\subcaption*{Alt text:  A figure summarising the estimated spatial structure in the point transect distance sampling data.  Sub-figure A shows the posterior detection function with associated uncertainty.  Detectability drops to just above 0.25 at the maximum observable distance.  Sub-figure B shows the posterior Mat\`ern correlation function for the random effect.  It shows a decreasing correlation as distance increases, dropping to a correlation around 0.1 at a distance of 10 kilometres.  Sub-figure C shows boxplots that summarise the posterior distribution of pairwise distances between observations simulated from the posterior predictive distribution.  It shows a very close match between the posterior mean and pairwise distances observed in the real observations.}
