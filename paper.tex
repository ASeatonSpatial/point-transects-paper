...
\end{equation*}
where $\bm{B}_{\bm{u}_0}$ is the derivative matrix of $\eta(\bm{u})$ evaluated at $\bm{u}_0$.  Since $\bar{\eta}_{\bm{u}_0}$ is linear in $\bm{u}$ it is possible to fit an approximate model in INLA by replacing the non-linear $\eta$ with the first-order Taylor approximation $\bar{\eta}_{\bm{u_0}}$.  This model configuration has $\bm{B}_{\bm{u}_0}$ as the model design matrix and $\eta(\bm{u}_0} - \bm{B}_{\bm{u}_0}\bm{u}_0$ as a known offset term.
...
\begin{figure}[!htb]
	\begin{center}
		\includegraphics[width=\textwidth]{figures/detfn_and_matern.pdf}
...
\begin{figure}[!htb]
	\begin{center}
		\includegraphics{figures/post_pp_distances.pdf}
		\caption{Boxplots of the frequency of pairwise distances based on 500 simulated datasets from the posterior predictive distribution.  Axis labels mark the largest value in each bin.   Red circles indicate the observed frequency of pairwise distances within each bin.}
