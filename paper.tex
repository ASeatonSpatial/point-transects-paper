...
Let $\bar{p}_{\bm{v}}$ denote the INLA approximate posterior distribution of the model parameters given the linearised model configuration at linearisation point $\bm{v}$.  New linearisation points are found by considering the functional
\begin{equation}
	f(\bar{p}_{\bm{v}}) = \argmax_{\bm{u}} \bar{p}_{\bm{v}}(u | \bm{Y})
\end{equation}
...
We then seek a fixed point of this functional so that $\bm{u}_0 = f(\bar{p}_{\bm{u}_0})$.  This fixed point is identified via an iterative scheme. Given a current choice of linearisation point $\bm{v}$, we estimate $\bar{p}_{\bm{v}}$ and set $\bm{u}_* :=f(\bar{p}_v)$ to use as the new linearisation point.  We iterate this procedure until a fixed point is identified within a chosen tolerance.
