...
The log-intensity for observed points (the thinned intensity) is $\log\tl(\bs) = \log 2 \pi r + \log\lambda(\bs) + \log g(\bs)$.  As we noted above, the detection function $g(\bs)$ is non-linear in its parameters and this component of the additive predictor requires a novel approach to parameter estimation.
